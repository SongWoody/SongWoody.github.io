...
최근 **Navigation 3**의 안정화 버전이 출시되었습니다. 이 글에서 새로운 API의 사용법을 나열하는 것을 넘어, **왜 이러한 변화가 필요했는지, 그리고 새로운 설계 원리가 어떻게 우리의 앱 아키텍처를 더 견고하고 유연하게 만드는지**를 살펴보겠습니다.

# Jetpack Navigation 3, 무엇이 달라졌고 왜 필요한가?
기존 Navigation 2는 `NavGraph`, `NavController`, `Destination` 같은 개념을 통해 화면 전환을 비교적 쉽게 만들어주었습니다. 하지만 Nav2 는 전통적인 액티비티, 프래그먼트를 지원하기 위해 탄생하면서 백스택을 직접 다루는 과정에서 복잡성이 발생하습니다. 또한 Nav2로 개발을 하게되면 다중 패널을 구현한데 한계가 있음을 느끼게 됩니다.

-   **백스택(Back Stack) 관리의 불투명성**: 개발자는 `navigate()`나 `popBackStack()` 같은 명령을 내릴 뿐, 백스택 내부가 어떻게 구성되고 동작하는지 직접 들여다보거나 제어하기 어려웠습니다. 이로 인해 Deep Link나 비선형적인 탐색 구조에서 예상치 못한 동작이 발생했을 때 디버깅이 매우 까다로웠습니다.
-   **다중 패널/폴더블 디바이스 지원의 어려움**: Nav2는 기본적으로 '하나의 화면'을 다른 '하나의 화면'으로 교체하는 방식에 최적화되어 있습니다. 태블릿이나 폴더블처럼 여러 화면을 동시에 보여주거나, 특정 영역만 동적으로 교체해야 하는 복잡한 UI를 구현하기에는 유연성이 부족했습니다.

Navigation 3는 바로 이러한 문제들을 해결하기 위해 등장했습니다. 기존의 불투명했던 구조를 벗어던지고, 개발자가 백스택의 상태를 명확하게 읽고 제어할 수 있는 **투명하고, 유연하며, 확장 가능한 새로운 아키텍처**를 제공합니다.
