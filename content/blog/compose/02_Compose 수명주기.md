...
컴포지션(Composition)은 컴포저블을 처음 실행할 때, UI를 설명하는 컴포저블 트리를 생성하는 과정입니다. 
...
그렇다면 컴포저블의 두 번째 수명 주기 단계인 **'0회 이상 재구성(Recompose)'** 에서 Compose가 어떤 인스턴스를 업데이트해야 할지, 혹은 건너뛰어야 할지 어떻게 알수 있을까요?  
